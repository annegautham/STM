...
//#define dt 40
int dt = 40;                              // Time step for scanning and PI control in microseconds
...
// Serial-related
void checkSerial();        // From SerialCommands.cpp

// Scan control
void incrementScan();      // STM scan incrementer
void waitTimeStep();       // Wait/delay function for scan timing

// Motor control
void stepMotors(int steps, int dir, int timePerStep);  // Motor stepping

// Math utilities
int saturate(int value, int maxVal, int minVal);       // Clamp function

int sigmaDelta(int in, int *sigma, unsigned int shift);   // Sigma-delta modulator
// Engage/logic
bool engage();  

/**************************************************************************/

bool manualEnabled = false;  // Default to false
